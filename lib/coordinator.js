...
Coordinator.prototype.setEditor = function (editor) {
	...
	this.editor.on('change', function (data) {
		...
		coordinator.emit('updateLastModificationDate', { lastModificationDate: new Date().toLocaleString()});
	});
...
Coordinator.prototype.setNetwork = function (network) {
	...
	this.network.on('receiveOps', function (data) {
		coordinator.emit('updateLastModificationDate', { lastModificationDate: data.lastModificationDate });
